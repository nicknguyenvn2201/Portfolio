...
def send_mail(recipient,ccRecipient, subject, message,lastmonth_month):  
    ...
    file_path = path / f'Malaysia Sales Order Data_{lastmonth_month}.xlsx'
    ...
    file_path2 = path / f'MY_AR Data_{lastmonth_month}.xlsx'
...
select extract(month from lead_date) as month, 
    ...
    cur = conn.cursor()
    my = pd.read_sql_query(q_my,conn)
    conn.commit()
    cur.close()
    conn.close()
    return my
...
@asset(group_name="send_commission_email_my")
def sending_email_commission_my(context: AssetExecutionContext) -> None:
    today = date.today()
    ...
    df.to_excel(os.path.join(path, f"Malaysia Sales Order Data_{lastmonth_month}.xlsx"))    
    df = ar_data_extract(lastmonth_date)
    df.to_excel(os.path.join(path, f"MY_AR Data_{lastmonth_month}.xlsx"))
    path_to = path / "ToCommissionMonthly_MY.txt"
    recipient = read_emails_commission_my_from_file(path_to)
    path_cc = path / "CcCommissionMonthly_MY.txt"
    ccRecipient = read_emails_commission_my_from_file(path_cc)
    subject = 'Malaysia Commission Raw Data for '+lastmonth_month
    message = '''Dear all,
    Please find attached to this email the Excel file of Malaysia Sales data & AR Data for the month.
    Thanks & Best Regards,'''
    send_mail(recipient,ccRecipient, subject, message,lastmonth_month)


send_email_commission_my_jobs = define_asset_job(
    name="send_email_commission_my_jobs",
    selection=AssetSelection.groups("send_commission_email_my"),
)

send_email_commission_my_schedule = ScheduleDefinition(
    job=send_email_commission_my_jobs,
    cron_schedule= "0 6 16 * *", 
